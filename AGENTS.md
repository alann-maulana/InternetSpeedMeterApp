...
- Resets counters when `rx/tx` values decrease unexpectedly (SpeedMonitor.swift:81-93)
- `NWPathMonitor` detects network path changes and triggers counter reset (SpeedMonitor.swift:49-57)
- Guards against underflow in session calculations (SpeedMonitor.swift:109-113)

### State Management
- Single `SpeedMonitor` instance created at app launch (InternetSpeedMeterAppApp.swift:10-12)
...
2. **Network byte counters can decrease** - happens on VPN disconnect or interface removal; code already handles this
3. **UserDefaults stores Int, not UInt64** - casting required (SpeedMonitor.swift:119-120, 129-130)
